...
impl Scraen {
    pub fn new(app: &App, params: ScraenDim, webcam_rect: Rect) -> Scraen {
        ...
        let fbo_rect = Rect::from_x_y_w_h(0.0, 0.0, fbo_resolution.0 as f32, fbo_resolution.1 as f32);
        ...
        let window_transform =
            Affine2::from_scale_angle_translation(draw_rect.wh() / fbo_rect.wh(), 0.0, draw_rect.xy());
    ...
    pub fn draw_eye(&self) {
        ...
        draw.ellipse().xy(self.eye_xy).radius(self.eye_r).color(WHITE);

        draw.rect().xy(self.eye_xy - rect_xy).wh(rect_wh).color(BLACK);
    ...
    pub fn render_texture(&mut self, app: &App) {
        ...
        if let Ok(image) = self.fbo.image_buffer.try_lock() {
            self.scraen_texture = wgpu::Texture::from_image(
                app,
                &image.resize_exact(self.scraen_resolution.0, self.scraen_resolution.1, FilterType::Gaussian),
            );
        }
    }
    ...
    pub fn serial_packet(&self) -> Option<Vec<u8>> {
        if let Ok(image) = self.fbo.image_buffer.try_lock() {
            let small_img = image
                .resize_exact(self.scraen_resolution.0, self.scraen_resolution.1, FilterType::Triangle)
                .rotate90();
