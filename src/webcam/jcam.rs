...
impl VideoCapture {
    pub fn new(
        ...
        // save size
        let width =
            capture.get(opencv::videoio::CAP_PROP_FRAME_WIDTH).unwrap();
        let height =
            capture.get(opencv::videoio::CAP_PROP_FRAME_HEIGHT).unwrap();
        ...
        // setup ring buffer
        let video_ring_buffer =
            RingBuffer::<Vec<Vec<opencv::core::Vec3b>>>::new(2);
        let (mut video_producer, video_consumer) =
            video_ring_buffer.split();
        ...
        let capture_thread = thread::spawn(move || {
            ...
            'capture: loop {
                ...
                // get usable data
                let data: Vec<Vec<opencv::core::Vec3b>> =
                    frame.to_vec_2d().unwrap();
                ...
                if let Ok(msg) = message_channel_rx.try_recv() {
                    match msg {
                        ...
                        Message::Pause(()) => {
                            ...
                            'pause: for message in
                                message_channel_rx.iter()
                            {
                                match message {
                                    Message::Close(()) => break 'capture,
                                    Message::SetSpeed(s) => {
                                        video_speed = s as f64
                                    }
                ...
                let elapsed =
                    clock.elapsed().unwrap().as_secs_f64() - start_time;
    ...
    pub fn update_texture(
        ...
        let image = image::ImageBuffer::from_fn(width, height, |x, y| {
            let pixel =
                self.frame_data[y as usize][(width - x - 1) as usize];
